...
describe('isNonFailoverError â€” validation must never fork engines', () => {
  const noFork: Array<[string, any]> = [
    ...
    ['sweep refusal', new Error('Page is not blank â€” refusing auto-sweep')],
...
describe('buildDivergence', () => {
  it('flags any table delta', () => {
    ...
    expect(dirty.tables.find((t) => t.table === 'pages')).toMatchObject({ tidb: 4, backup: 5, delta: -1 })
  })
})
...
describe('getStorageShiftStatus with operator override (no live probe)', () => {
  ...
  it('shifts books under 10MB and reports override source', () => {
    ...
    expect(s.books.shiftedToBackup).toBe(true)
    expect(s.books.quotaSource).toBe('override')
    expect(s.notes.shiftedToBackup).toBe(false)
  })
